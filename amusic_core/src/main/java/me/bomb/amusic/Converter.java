...
final class Converter implements Runnable {
	...
	private static final String fmpegbinarypath, binarywithargs;
	...
	static {
		...
		binarywithargs = " -acodec libvorbis -ab ".concat(Integer.toString(64000)).concat(" -ac ").concat(Integer.toString(2)).concat(" -ar ").concat(Integer.toString(44100)).concat(" -f ogg");
	}
	...
	public void run() {
		try {
			try {
				...
				StringBuffer sb = new StringBuffer(fmpegbinarypath);
				sb.append(" -i ");
				sb.append(input.getAbsolutePath());
				sb.append(binarywithargs);
				sb.append(" -y ");
