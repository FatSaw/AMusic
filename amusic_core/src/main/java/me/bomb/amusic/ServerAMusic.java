...
public final class ServerAMusic extends LocalAMusic implements Runnable {
	...
	@Override
	public final boolean loadResourcepack(UUID[] playeruuid, String name, boolean update, Consumer<LoadPackResult> resultConsumer) {
		if(update) {
			this.cacheupdatemarker.clear();
		}
		return super.loadResourcepack(playeruuid, name, update, resultConsumer);
	}
	
	public final void processLoadResourcepack(InputStream is, OutputStream os) throws IOException {
	...
	public final void processSetRepeat(InputStream is, OutputStream os) throws IOException {
	...
	public final void processGetLoadedPlayers(InputStream is, OutputStream os) throws IOException {
		byte[] buf = new byte[0x100];
		int off = 0;
		while(off < 1) {
	...
	public final void processGetLoadedResourcepackName(InputStream is, OutputStream os) throws IOException {
		byte[] buf = new byte[0x10];
		int off = 0;
		while(off < buf.length) {
			int n = is.read(buf, off, buf.length - off);
	...
	public final void processGetResourcepackInfoList(InputStream is, OutputStream os) throws IOException {
	...
	public final void processGetSourceResourcepackNameList(InputStream is, OutputStream os) throws IOException {
	...
	public final void processGetSourceSoundnameList(InputStream is, OutputStream os) throws IOException {
		byte[] buf = new byte[0xFF];
		int off = 0;
		while(off < 1) {
			int n = is.read(buf, off, 1 - off);
	...
	private final void processConnection(Socket connected) throws IOException {
		...
		if(packetid == 0x01) {
			processLoadResourcepack(is, connected.getOutputStream());
			return;
		}
		if(packetid == 0x02) {
		...
		if(packetid == 0x04) {
			processSetRepeat(is, connected.getOutputStream());
			return;
		}
		if(packetid == 0x05) {
		...
		if(packetid == 0x06) {
			processGetLoadedPlayers(is, connected.getOutputStream());
			return;
		}
		if(packetid == 0x07) {
			processGetLoadedResourcepackName(is, connected.getOutputStream());
			return;
		}
		
		if(packetid == 0x08) {
			processGetResourcepackInfoList(is, connected.getOutputStream());
			return;
		}
		if(packetid == 0x09) {
		...
		if(packetid == 0x0B) {
			processGetSourceResourcepackNameList(is, connected.getOutputStream());
			return;
		}
		if(packetid == 0x0C) {
			processGetSourceSoundnameList(is, connected.getOutputStream());
			return;
		}
	}
