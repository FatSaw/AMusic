...
public final class LocalSoundSource implements SoundSource<SourceEntry> {
	...
	static {
		...
		InputStream is = LocalSoundSource.class.getClassLoader().getResourceAsStream("silence.ogg");
		try {
			int off = 30;
			while(off < size) {
	...
	public LocalSoundSource(Path musicdir, int maxresourcepacksize, int maxsoundsize, float threadcoefficient, short threadcountlimit) {
	...
	public PackedResourcepack get(String entrykey, PackMergeEntry packmerge) {
		...
		HashMap<Path, Integer> filesm = null;
		try {
		...
		while(--i > -1) {
			...
			LocalSoundSource.ReadSound run = new LocalSoundSource.ReadSound(this.fsp, file, (short)i, resourcepack, offset, size, globalheaderoffset, soundsjsonentryoffset, sounddefenitionsentryoffset, soundname, soundhashs, lengths);
			queue.add(run);
			offset += size;
		...
		ResourcepackInfoImpl info = new ResourcepackInfoImpl(infosize, resourcepack.length, entrykey, sounds, sha1hash.digest(), sha256hash.digest(), bhea, bres, null);
	...
	public static final class PackedResourcepack implements SourceEntry {
		...
		private PackedResourcepack(byte[] resourcepack, ResourcepackInfoImpl info) {
			this.resourcepack = resourcepack;
			this.info = info;
		}
	}
	
	protected static final class ReadSound implements Runnable {
		...
		private ReadSound(FileSystemProvider fsp, Path file, short num, byte[] resourcepack, int offset, int length, int globalentryoffset, int soundsjsonentryoffset, int sounddefenitionsentryoffset, byte[] soundname, UUID[] soundhashs, short[] lengths) {
			this.fsp = fsp;
			this.file = file;
			this.num = num;
			this.resourcepack = resourcepack;
