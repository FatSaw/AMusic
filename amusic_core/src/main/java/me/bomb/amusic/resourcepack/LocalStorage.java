...
public class LocalStorage extends me.bomb.amusic.resourcepack.Data {
	...
	protected LocalStorage(boolean lockwrite, LocalSoundSource lczs, PackMergeSource pms, Path packeddirectory) {
		super(lockwrite);
		this.lczs = lczs;
		this.pms = pms;
	...
	public void load() {
		...
		try {
			...
			while(it.hasNext()) {
				...
				LocalDataEntry entry = loadAmp(storeid);
				if(entry == null) {
					continue;
				}
	...
	private LocalDataEntry loadAmp(String storeid) {
		if(storeid == null) {
		...
		return new LocalDataEntry(storeid, info, ampifile);
	}
	
	private DataEntry saveAmp(String storeid, ResourcepackInfoImpl info, byte[] resource) {
