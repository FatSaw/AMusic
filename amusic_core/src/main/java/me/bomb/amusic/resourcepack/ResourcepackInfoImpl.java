...
public final class ResourcepackInfoImpl implements ResourcepackInfo {
	...
	protected ResourcepackInfoImpl(int infosize, int packsize, String packname, SoundInfo[] sounds, byte[] sha1, byte[] sha256, UUID bhea, UUID bres, byte[] customdata) {
	...
	public static int serialize(OutputStream os, ResourcepackInfoImpl info) throws IOException {
	...
	public static ResourcepackInfoImpl deserialize(InputStream is) throws IOException {
		if(is == null) {
			throw new IllegalArgumentException();
		}
		int infosize = 102;
		...
		if(customdatalength > 0) {
			...
			off = 0;
			while(off < customdata.length) {
				int n = is.read(customdata, off, customdata.length - off);
				if (n == -1) {
			        throw new EOFException();
			    }
				off += n;
		...
		return new ResourcepackInfoImpl(infosize, packedsize, packedname, sounds, sha1, sha256, bhea, bres, customdata);
	}
