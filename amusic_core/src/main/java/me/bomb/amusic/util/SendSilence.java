...
public final class SendSilence implements Runnable {
	...
	volatile byte remaining;

	public SendSilence(final Channel channel, final int version) {
		ByteBuf buf;
		if (version < 4 || version > 110 || (buf = version < 48 ? legacysoundstop1 : version > 106 ? legacysoundstop2 : null) == null) {
	...
	public void run() {
		...
		this.channel.writeAndFlush(this.buf.retainedDuplicate(), this.voidpromise);
		if(--this.remaining < 1) {
			return;
		}
